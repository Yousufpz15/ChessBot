...
def has_square_image_changed(old_square, new_square):#If there has been a change -> the image difference will be non null -> the average intensity will be > treshold
    ...
    if diff.mean() > 6: #8 works pretty nicely but would require optimization
        return True
    else:
        return False

def is_square_empty(square): # A square is empty if its pixels have no variations
    return square.std() < 14 # 10 works pretty well -> the mouse pointer is not enought to disturb (but sometimes it actually does, especially with small chessboards and big pointer)
